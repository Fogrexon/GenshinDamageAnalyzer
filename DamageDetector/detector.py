...
class Detector:
  ...
  def get_damage_outline(self, image, key):
    height, width, _ = image.shape
    resized = cv2.resize(image, (int(width * self.scale), int(height * self.scale)))
    binarized = self.binarize(resized, key)
  ...
  def get_damages(self, image, key):
    outlines = self.get_damage_outline(image, key)
    result = []
    for rect in outlines:
      x, y, w, h = rect
      
      cropped = image[y:y+h, x:x+w, :]
      num = self.get_number(cropped, key)
      if num < 1000:
        continue
      result.append({
        "number": num,
